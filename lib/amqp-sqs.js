...
Queue.prototype.subscribe = function(options, listener) {
  ...
  var self = this;
  var pollInterval = (options.pollInterval || AMQP_CONFIG.subscribe.pollInterval) * 1000 * 60;

  if (pollInterval) {
    console.log('[amqp-sqs]: set poll interval to ' + pollInterval + ' ms');
  } else {
    console.log('[amqp-sqs]: no poll interval set');
  }
  ...
  var fn = function X() {
    ...
    sqs.receiveMessage(self._name, function(err, res) {
      console.log('[amqp-sqs]: message received in ' + String(new Date() - timestamp) + ' ms');
            ...
            _.each(updates, function(update) {
              ...
              self._limiter.removeTokens(1, function(/* err, remainingRequests */) {
                listener(update, function(done) {
                  console.log('[amqp-sqs]: processed message #' +
                    (updates.length - count + 1) + ' after ' +
                    String(new Date() - timestamp) + ' ms');
                  ...
                  if (!--count) {
                    console.log('[amqp-sqs]: processed all messages after ' +
                      String(new Date() - timestamp) + ' ms');
      ...
      console.log('[amqp-sqs]: finished processing received messages in ' + elapsed + ' ms');

      wait = Math.max(0, pollInterval - elapsed);
      if (wait > 0) {
        console.log('[amqp-sqs]: about to wait for ' + wait + ' ms');
      }
      setTimeout(fn, wait);
    });
  };
